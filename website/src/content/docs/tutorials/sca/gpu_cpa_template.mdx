...
trace length was 80,000 and our batch size is 100 then `batch[“trace1”]` is a
...
Our choice of representation – one aggregate per hypothesis value – is not well
...
[The Sharp Bits 🔪 — JAX documentation](https://docs.jax.dev/en/latest/notebooks/Common_Gotchas_in_JAX.html#in-place-updates).
...
[Just-in-time compilation — JAX documentation](https://docs.jax.dev/en/latest/jit-compilation.html)
in action where it made our code run faster. From implementing neural networks
you might remember the
[Automatic differentiation — JAX documentation](https://docs.jax.dev/en/latest/automatic-differentiation.html).
The last one is
[Automatic vectorization — JAX documentation](https://docs.jax.dev/en/latest/automatic-vectorization.html#automatic-vectorization)
which makes code run on higher dimensional arguments. The beauty of these and
other JAX operations is that they compose – that is one can for example jit a
...
[Named Tuples — typing documentation](https://typing.python.org/en/latest/spec/namedtuples.html)
instances. We could also pass a dictionary or a completely custom class
provided we satisfy JAX’s very mild expectations of their API. For more
information see
...
[Automatic vectorization — JAX documentation](https://docs.jax.dev/en/latest/automatic-vectorization.html)
...
to use
[scipy.stats.multivariate\_normal — SciPy v1.16.2 Manual](https://docs.scipy.org/doc/scipy/reference/generated/scipy.stats.multivariate_normal.html)
...
[GPU memory allocation — JAX documentation](https://docs.jax.dev/en/latest/gpu_memory_allocation.html).

If one is using TensorFlow shard files (“tfrec”) they would also benefit from
iterating inside a CPU context using: `with tf.device(“/CPU”):` as noted by
...
[Default dtypes and the X64 flag — JAX documentation](https://docs.jax.dev/en/latest/default_dtypes.html).
Another choice would be to use more numerically precise methods such as the
